...
def push_to_kafka(event, topic, config):
    producer = Producer(config)
    producer.produce(topic, json.dumps(event).encode('utf-8'))
    producer.flush()


def gen_clickstream_data(num_click_records: int, config: dict) -> None:
    for _ in range(num_click_records):
        user_id = random.randint(1, 100)
        click_event = generate_click_event(user_id)
        push_to_kafka(click_event, 'clicks', config)
...
if __name__ == "__main__":
    ...
    gen_clickstream_data(args.num_click_records, config)
